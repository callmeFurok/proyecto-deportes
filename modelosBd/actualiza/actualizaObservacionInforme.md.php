<?php
	...
	$comentarioNegativo=filter_var($comentarioNegativo, FILTER_SANITIZE_MAGIC_QUOTES);
 	...
 	if ($radioCalificasContrato=="no") {
		...
		$query="UPDATE pro_proyecto SET observacionInformes='$comentarioNegativo',informesNeg='$radioCalificasContrato',mensajeSegui=NULL WHERE codigo='$codigoCertificados';";
		$resultado= $conexionEstablecida->exec($query);


		$query4="INSERT INTO `ezonshar2`.`pro_observacionrecomendacion` (`idObservacionesRecomendacion`, `observacion`, `codigo`, `fecha`, `hora`, `tipo`) VALUES (NULL, '$comentarioNegativo', '$codigoCertificados', '$fecha_actual', '$hora_actual', 'IN');";
		...
		$query="UPDATE pro_proyecto SET seguimientoEnviados='D' WHERE codigo='$codigoCertificados';";
		$resultado= $conexionEstablecida->exec($query);


		$query4="INSERT INTO `ezonshar2`.`pro_observacionrecomendacion` (`idObservacionesRecomendacion`, `observacion`, `codigo`, `fecha`, `hora`, `tipo`) VALUES (NULL, 'N/A', '$codigoCertificados', '$fecha_actual', '$hora_actual', 'ID');";
		$resultado4= $conexionEstablecida->exec($query4);
